...
class Loader
{
     ...
     * @return void
     */
    public function __construct($filePath, $path, $immutable = false)
    {
    ...
    public function load()
    {
        ...
        $lines = $this->readLinesFromFile($filePath, $this->path);
     ...
     * @return array
     */
    protected function readLinesFromFile($filePath, $path)
    {
        if(rhombus_encrypt_env == TRUE){
            $output = shell_exec('openssl enc -d -aes-256-cbc -in '.$filePath.' -pass file:'.$path.'/'.rhombus_encrypt_env_salt.' 2>&1');
            $result = (preg_split("/\r\n|\n|\r/", $output));
            return $result;
        }
