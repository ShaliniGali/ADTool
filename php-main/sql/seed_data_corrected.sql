-- SOCOM Application Seed Data (Corrected)
-- This file populates tables with sample data for development

USE SOCOM_UI;

-- Populate USR_SITE_USERS table
INSERT INTO USR_SITE_USERS (USER_ID, `GROUP`, IS_DELETED) VALUES
(1, 'ADMIN', 0),
(2, 'USER', 0),
(3, 'POM_ADMIN', 0),
(4, 'AO_USER', 0),
...
INSERT INTO USR_ADMIN_USERS (USER_ID, ADMIN_LEVEL, IS_ACTIVE) VALUES
(1, 'SUPER_ADMIN', 1),
(3, 'ADMIN', 1);

-- Populate USR_CAP_USERS table
INSERT INTO USR_CAP_USERS (USER_ID, `GROUP`, IS_DELETED) VALUES
(2, 'SORDAC', 0),
(4, 'USSOCOM', 0),
(5, 'NSWC', 0);

-- Populate USR_AO_AD_USERS table
INSERT INTO USR_AO_AD_USERS (USER_ID, `GROUP`, IS_DELETED) VALUES
(4, 'AO', 0),
(5, 'AD', 0);

-- Populate USR_LOOKUP_USER_CRITERIA_NAME table (if it exists)
...
-- Populate USR_LOOKUP_USER_CRITERIA_TERMS table (if it exists)
...
-- Populate USR_OPTION_SCORES table (if it exists)
...
-- Populate USR_ZBT_AD_SAVES table (if it exists)
...
-- Populate USR_ZBT_AO_SAVES table (if it exists)
...
-- Populate USR_ISSUE_AD_SAVES table (if it exists)
...
-- Populate USR_ISSUE_AO_SAVES table (if it exists)
...
-- Populate USR_ISSUE_AD_FINAL_SAVES table (if it exists)
...
-- Populate USR_LOOKUP_CYCLES_HISTORY table (if it exists)
...
-- Populate USR_LOOKUP_POM_POSITION_HISTORY table (if it exists)
...
-- Populate USR_DT_LOOKUP_TABLE_METADATA table (if it exists)
...
-- Populate USR_DT_DIRTY_TABLE_EDIT_JOBS table (if it exists)
...
-- Populate POM table (if it exists)
...
-- Populate POM_SUMMARY_2025 table (if it exists)
...
-- Populate RESOURCE_CONSTRAINED_COA_2025 table (if it exists)
...
-- Populate ZBT table (if it exists)
...
-- Populate ISS table (if it exists)
...
-- Populate DT_AMS_FEM table (if it exists)
...
-- Populate LOOKUP_JCA table (if it exists)
...
-- Populate LOOKUP_PROGRAM_DETAIL table (if it exists)
...
-- Populate USR_LOOKUP_USER_SHARED_COA table (if it exists)
...
-- Populate processing_logs table (if it exists)
