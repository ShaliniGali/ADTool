...
function get_budget_to_execution_graph(id, data) { 

    fullSeriesData[id] = JSON.parse(JSON.stringify(data['data']));
   
    Highcharts.chart(id, {
        title: {
            text: `Budget & Execution from ${data['categories'][0]} to ${data['categories'][data['categories'].length - 1]}`,
        },
        xAxis: {
...
function dropdown_onchange(id, type, row_id = null) {
    ...
    switch(type) {
        ...
        case 'filter': // onclick for 'Apply Filter'
            ...
            $('#budget-to-execution-filter-compare').attr('disabled', false);

            $('#chart-2-container').addClass('d-none').removeClass('d-flex');
            $('#chart-1-container').addClass('w-100').removeClass('w-50');
            $('#list-1').empty();

            update_budget_to_execution_graph(id, input_object, 'chart-1', function() {
                update_graph(id, $('#chart-1'));
                // reset toggle
                $('#budget-to-execution-common-range-toggle-container').attr('hidden', true);
                $('#budget-to-execution-common-range-toggle').prop('checked', false);
        ...
        case 'compare':
            ...
            update_budget_to_execution_graph(id, input_object, 'chart-2',function() {
                ...
                $('#budget-to-execution-common-range-toggle-container').attr('hidden', false);
                $('#budget-to-execution-common-range-toggle').prop('checked', false).trigger('change');
                resetYearSlider(2)
    ...
    $("#year-slider-"+id).ionRangeSlider({
        ...
        onChange: function (data) {
            ...
            chart.update({
                ...
                title: {
                    text: `Budget & Execution from ${filteredCategories[0]} to ${filteredCategories[filteredCategories.length - 1]}`
                }
            }, true);
        },
    });
...
function check_filter_limit(type, id, limit, compareFlag) {
    ...
    if ($dropdown.val().length > limit) {
        if ($filterError.length === 0) {
            ...
            $(`#${type}-dropdown`).append(
                `<div id="${filterErrorId}" class="alert alert-warning mb-0 mt-2" style="width: 17wv;">
...
function update_budget_to_execution_graph(id, input_object, chart_id, callback=null) {
    input_object = get_input_object(id);
    ...
    if (allFiltersSelected) {
        $.post("/socom/budget_to_execution/graph/update", { 
            ...input,
...
function showGraph(graphData, chart_id) {
    ...
    if (allSum === 0 || graphData.data.length === 0) {
        ...
        get_budget_to_execution_graph(chart_id, graphData)
    }
}
...
async function update_resource_category_filter(id, prevFilterWithinLimit) {
    ...
    if (previousFiltersSelected && prevFilterWithinLimit) {
        try {
            ...
            const data = await $.post("/socom/budget_to_execution/filter/resource_category/update", {
                rhombus_token: rhombuscookie(),
                ...input,
            })
...
async function update_program_filter(id, prevFilterWithinLimit) {
    ...
    if (previousFiltersSelected && prevFilterWithinLimit) {
        try {
            ...
            const data = await $.post("/socom/budget_to_execution/filter/program/update", {
                rhombus_token: rhombuscookie(),
                ...input,
            })
...
async function update_execution_manager_filter(id, prevFilterWithinLimit) {
    ...
    if (previousFiltersSelected && prevFilterWithinLimit) {
        try {
            ...
            const data = await $.post("/socom/budget_to_execution/filter/execution_manager/update", {
                rhombus_token: rhombuscookie(),
                cs: input_object["cs"],
                'ass-area': input_object["ass-area"]
            })
...
async function update_program_name_filter(id, prevFilterWithinLimit) {
    ...
    const previousFiltersSelected = input_object["ass-area"] && input_object["cs"] && input_object["execution-manager"] && input_object["program"];

    if (previousFiltersSelected && prevFilterWithinLimit) {
        try {
            $(`#program-name-${id}`).attr('disabled', true);
            $('#program-name-loading').attr('hidden', false);
            const data = await $.post("/socom/budget_to_execution/filter/program_name/update", {
                rhombus_token: rhombuscookie(),
                cs: input_object["cs"],
                'ass-area': input_object["ass-area"],
...
async function update_eoc_code_filter(id, prevFilterWithinLimit) {
    ...
    if (previousFiltersSelected && prevFilterWithinLimit) {
        try {
            ...
            const data = await $.post("/socom/budget_to_execution/filter/eoc_code/update", {
                rhombus_token: rhombuscookie(),
                cs: input_object["cs"],
                'ass-area': input_object["ass-area"],
                'execution-manager': input_object["execution-manager"],
...
async function update_osd_pe_filter(id, prevFilterWithinLimit) {
    ...
    if (previousFiltersSelected && prevFilterWithinLimit) {
        try {
            ...
            const data = await $.post("/socom/budget_to_execution/filter/osd_pe/update", {
                rhombus_token: rhombuscookie(),
                cs: input_object["cs"],
                'ass-area': input_object["ass-area"],
                'execution-manager': input_object["execution-manager"],
...
function handle_disable_apply_filter_button(id) {
    ...
    const allFiltersWithinLimit = allFiltersShown
        ? allFilterTypes.every(type => check_filter_limit(type, id, 5))
        : defaultFilterTypes.every(type => check_filter_limit(type, id, 5));

    if (allFiltersSelected && allFiltersWithinLimit) {
        $('#budget-to-execution-filter').attr('disabled', false);
    }
    else {
        $('#budget-to-execution-filter').attr('disabled', true);
    }
}
...
function common_range_toggle() {
    if($('#budget-to-execution-common-range-toggle').prop('checked')) {
...
window._rb.get_budget_to_execution_graph = get_budget_to_execution_graph;
window._rb.dropdown_onchange = dropdown_onchange;
window._rb.update_budget_to_execution_graph = update_budget_to_execution_graph;
...
window._rb.selectionHasChanged = selectionHasChanged;
