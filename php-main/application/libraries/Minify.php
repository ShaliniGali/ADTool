...
class Minify
{
	 ...
	 * @var array
	 */
	public $closurecompiler = array('compilation_level' => 'ADVANCED_OPTIMIZATIONS');
	...
	private function _config_checks()
	{
		...
		if ( ! empty($this->assets_dir_css) && ! is_writable($this->assets_dir_css))
		{
			/*
				...
				Checks if the css_compress folder exists, creates it otherwise.
			*/
			$oldmask = umask(0);
			if (!mkdir($this->assets_dir_css, 0777)) {
		...
		{	
			/*
				...
				Checks if the js_compress folder exists, creates it otherwise.
			*/
			$oldmask = umask(0);
			if (!mkdir($this->assets_dir_js, 0777)) {
