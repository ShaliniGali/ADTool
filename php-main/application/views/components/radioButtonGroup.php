...
<fieldset class="<?=$useTile ? 'bx--tile-group' : 'bx--fieldset' ?>">
    ...
    <div class="bx--radio-button-group <?= $radioGroupClass ?>">
        ...
        <div class="bx--radio-button-wrapper">
          <input id="<?= $button['id'] ?>" class="<?= $useTile ? 'bx--tile-input' : 'bx--radio-button' ?>" type="radio" value="<?= $button['value'] ?>" name="<?= ($name ?? 'radio-button') ?>" tabindex="<?= $key ?>" <?php echo (isset($button['checked']) && $button['checked'])? 'checked': '' ?>>
          <label for="<?= $button['id'] ?>"  class="<?= $useTile ? 'bx--tile bx--tile--selectable' : 'bx--radio-button__label' ?> <?= $useTile && isset($button['checked']) &&  $button['checked'] ? 'bx--tile--is-selected' : '' ?>">
            <?php if ($useTile === true): ?>
            ...
            <?php endif; ?>
            <span class="<?= $useTile ? 'bx--tile-content' : 'bx--radio-button__label-text' ?>"><?= $button['label'] ?></span>
          </label>
        </div>
        <?php endforeach;?>
    </div>
