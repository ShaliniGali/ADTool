<style>
    ...
    .handsontable.listbox tr td {
        background-color: #362511 !important;
    ...
    #upload-merge-btn.bx--btn--sm {
        height: 50px !important;
        width: 175px !important;
    }
    #view-status-btn.bx--btn--sm {
        height: 50px !important;
        width: 175px !important;
    }
...
<?php 
    ...
    $js_files['editor_merge_recent_view'] = ['actions/SOCOM/dashboard/upload/editor_merge_recent.js', 'custom'];
    $js_files['editor_view'] = ['actions/SOCOM/dashboard/upload/editor.js', 'custom'];
    $js_files['highstock'] = ["highstock.js", 'global'];
    $js_files['timeline'] = ["timeline.js", 'global'];
...
<div class="d-flex flex-row justify-content-between align-items-start search-bar-wrapper" style="width: 100%;">
    ...
    <h1 class="mt-3 mr-5">
    <?= $admin_viewer === '/1' ? 'Read Only Mode' : 'Editor Mode' ?>
    </h1>
...
<div class="d-flex flex-row justify-content-between align-items-center flex-wrap" style="width:100%">
     <div class="d-flex flex-row ml-3 mt-4" style="gap: 20px;">
        <button class="bx--btn bx--btn--primary bx--btn--sm" type="button" id="view-status-btn">
            View Status
        </button>
        <button class="bx--btn bx--btn--primary bx--btn--sm" type="button" id="view-history-btn">
            View History Chart
        </button>
    </div>
...
<div class="d-flex flex-row sv-cl-button-wrapper mt-4">
    <div class="bx--btn-set">
        <?php if ($is_admin): ?>
        <button class="bx--btn bx--btn--primary bx--btn--sm mr-2" type="button" id="upload-merge-btn">
                    Admin Merge
        </button>
        <?php endif; ?>
        ...
        <button class="bx--btn bx--btn--primary bx--btn--sm mr-2" type="submit" id="save-edits-btn">
                    Save
        </button>
        <?php if ($is_admin): ?>
        <button class="bx--btn bx--btn--primary bx--btn--sm" type="button" id="approve-row-btn">
                    Approve
        </button>
        <?php endif; ?>
    </div>
...
$this->load->view('templates/carbon/carbon_modal', [
    'modal_id' => 'editor_merge_recent_modal',
    'role' => 'editor-merge-role',
    'title' => 'Editor Merge',
    'title_heading' => 'Merge Conflict Details',
    'html_content' => $this->load->view('SOCOM/dashboard/upload/zbt_iss_editor/editor_merge_recent_modal', 
    [], true),
    'buttons' => [
        [
...
<?php $this->load->view('SOCOM/dashboard/upload/view_status_modal'); ?>
<?php $this->load->view('SOCOM/dashboard/upload/view_history_modal'); ?>
<script>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (viewStatusBtn && modal && closeModalBtn) {
            viewStatusBtn.addEventListener('click', () => {
                statusTextArea.textContent = 'Status: No unsaved changes.'; 
        ...
        const historyModal = document.getElementById('view-history-modal');
        const closeHistoryModalBtn = document.getElementById('close-history-modal');

        if (viewHistoryBtn && historyModal && closeHistoryModalBtn) {
            viewHistoryBtn.addEventListener('click', () => {
                fetchEditorHistoricalGraphData(() => {
