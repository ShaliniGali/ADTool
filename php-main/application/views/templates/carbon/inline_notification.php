<div id="<?=$notification_id?>" data-notification
  class="bx--inline-notification bx--inline-notification--<?= $type ?> <?= ((!isset($dnone) || $dnone !== false) ? 'd-none' : '')?>"
  role="alert">
  <div class="bx--inline-notification__details">
    ...
    <div class="bx--inline-notification__text-wrapper">
      <p class="bx--inline-notification__title"><?= $bx_notification_title ?? "" ?></p>
      <p class="bx--inline-notification__subtitle"><?= $bx_notification_subtitle ?? "" ?></p>
    </div>
  </div>
...
if ($close === true) { 
        $closeScript = <<<EOT
$('#{$notification_id} > button.bx--inline-notification__close-button')
  .on('click', function() { 
    $('#{$notification_id}').addClass('d-none'); 
  });
EOT;
} else if (is_string($close)) {
  $closeScript = <<<EOT
$('#{$notification_id} > button.bx--inline-notification__close-button')
  .on('click', function() { 
    {$close}
...
if (isset($action)) {
    $actionScript = <<<EOT
$('#{$notification_id} > button.bx--inline-notification__action-button').on('click', {$action});
EOT;
} else {
    $actionScript = "";
}
