...
class Database_Save_Out_of_POM_Cycle_Data_Upload extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->library('SOCOM/Out_of_POM_Cycle_Data_Upload_Import');
    }
    ...
    public function save_upload()
    {
        ...
        if ($is_pom_admin === false) {
            ...
            $this->form_validation->set_rules('table-listing', 'Table Listing', 'required|in_list[PB,ENT,ACTUALS]');
            ...
            if ($this->form_validation->run() !== false && $data_check['result']) {
                ...
                try {
                    $this->out_of_pom_cycle_data_upload_import->setParams([
                        'VERSION' => $version,
                        'TITLE' => $title,
                        'DESCRIPTION' => $description,
                        'POM_YEAR' => $year,
                        'TABLE_LISTING' => $table_listing
                    ], UploadType::DT_OUT_POM);

                    $this->out_of_pom_cycle_data_upload_import->setFilePostName('file');

                    $status = $this->out_of_pom_cycle_data_upload_import->saveUpload();

                    $this->out_of_pom_cycle_data_upload_import->saveToMetaDatabase();
    ...
    public function list_uploads() {
        ...
        if ($is_pom_admin === false) {
            ...
            if ($status !== FALSE) {
                ...
                if (($file_list = $this->SOCOM_Database_Upload_Metadata_model->get_file_upload_list($user_id, UploadType::DT_OUT_POM)) !== false) {
                    $http_status = 200;
    ...
    public function get_processed() {
        ...
        if ($is_pom_admin === false) {
            ...
            $data = $this->SOCOM_Database_Upload_Metadata_model->get_processed_list_pom_status(UploadType::DT_OUT_POM);
            $response = ['data' => $data];
