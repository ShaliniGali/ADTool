...
class SOCOM_Dashboard_Site_Users extends CI_Controller
{
    ...
    public function get_pom_user_list()
    {
        $users = $this->SOCOM_Users_model->get_users();

        try {
            $users = $this->SOCOM_Site_User_model->get_users($users);
    ...
    public function save_pom_status()
    {
        $admin_users = false;
        ...
        if ($this->form_validation->run() !== FALSE && $data_check['result']) {
            ...
            if ($sid === 1) {
                $admin_users = $this->SOCOM_Site_User_model->delete_user($user_id);
            } else {
                $admin_users = $this->SOCOM_Site_User_model->activate_user($user_id);
            }
    ...
    public function save_my_user_pom()
    {
        ...
        $users = $this->SOCOM_Site_User_model->set_user($user_id, (int)$gid);
