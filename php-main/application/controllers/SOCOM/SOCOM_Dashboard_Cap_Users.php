...
class SOCOM_Dashboard_Cap_Users extends CI_Controller
{
    ...
    public function get_cap_user_list()
    {
        $users = $this->SOCOM_Users_model->get_users();

        try {
            $users = $this->SOCOM_Cap_User_model->get_users($users);
    ...
    public function save_cap_status()
    {
        $cap_users = false;
        ...
        if ($this->form_validation->run() !== FALSE && $data_check['result']) {
            ...
            if ($sid === 1) {
                $cap_users = $this->SOCOM_Cap_User_model->delete_user($user_id);
            } else {
                $cap_users = $this->SOCOM_Cap_User_model->activate_user($user_id);
            }
        ...
        if ($cap_users === false) {
            ...
            $http_status = 200;
            $output = json_encode(['status' => $cap_users]);
        }
    ...
    public function save_my_user_cap()
    {
        ...
        $users = $this->SOCOM_Cap_User_model->set_user($user_id, $gid);

        $site_users = $this->SOCOM_Site_User_model->set_user($user_id, 3);
        $site_users = $this->SOCOM_Site_User_model->activate_user($user_id, true);

        if ($users === false || $site_users === false) {
            $http_status = 500;
        } else {
            $http_status = 200;
        }
        ...
        $this->output
            ->set_status_header($http_status)
            ->set_content_type(self::CONTENT_TYPE_JSON)
            ->set_output(json_encode(['status' => $site_users === false ? $site_users: $users]));
    }
}
