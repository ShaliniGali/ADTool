...
class ExcelValidator:
    ...
    def validate_program_id(self, schema, conn):
        ...
        query = f'''SELECT DISTINCT ID FROM {schema}.LOOKUP_PROGRAM WHERE EVENT_NAME IS NULL;'''
        ...
        # raising error on duplicated PIDs, not applicable in BASE DT UPLOAD        
    ...
    def validate_fiscal_year(self, file_id, schema, conn):
        # check year is four characters in string format
        # POM Year is min, POM Year + 4 is max.
        query = f'''SELECT POM_YEAR FROM {schema}.USR_DT_LOOKUP_METADATA WHERE USR_DT_UPLOAD_ID = {file_id}'''
    ...
    def run_all_validators(self, errors, file_id, schema, conn):
        ...
        try:
            for method, method_desc in [
                ...
                # (lambda: self.validate_unique(), 'No Duplicate Column Subsets'), #funding lines can be of same years
                ...
                (lambda: self.validate_cols_not_negative(), 'Work Years - Not Negative'),
...
def get_job_rows(conn):
    ...
    try:
        ...
        sql = f"""
            ...
            WHERE u.`TYPE` = 'DT_UPLOAD_BASE_UPLOAD'
                AND s.CRON_STATUS = 0 
                AND u.FILE_STATUS = 1;
        """
...
def get_file_stream(s3_client, BUCKET, file_path):
    """Returns a BytesIO stream of the file from Minio or boto3 S3 client"""
    if isinstance(s3_client, Minio):
        # Minio uses positional args, returns HTTPResponse
        response = s3_client.get_object(BUCKET, file_path)
        content = response.read()
        response.close()
        response.release_conn()
    else:
        # boto3 returns dict with 'Body' stream
