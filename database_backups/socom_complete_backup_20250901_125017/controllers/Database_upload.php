...
class Database_Upload extends CI_Controller {
    ...
    public function __construct()
    {
        ...
        if (!$dev_bypass_enabled) {
            ...
            if(!$is_restricted && !$is_admin && !$is_guest) {
                $http_status = 403;
    ...
    public function index() {
        ...
        $data['fiscal_years_out_of_pom'] = $this->SOCOM_Database_Upload_model->get_fiscal_years_out_of_pom_upload();
    ...
    public function process_file(){
        $data_check = $this->DB_ind_model->validate_post($this->input->post()); //validating input posts
            ...
            if ($data_check['result']) {
                ...
                if (in_array($tab_name, $tab_list) && ($is_pom_admin === false && $is_guest === false && $is_restricted === false)) {
                    $http_status = 403;
                    ...
                    if ($status) {
                        ...
                        $this->SOCOM_Git_Data_model->git_track_data(GitDataType::PROCESS_FILE, $file, $user_id);
        ...
        if ($data_check['result']) {
            ...
            if (in_array($tab_name, $tab_list) && ($is_pom_admin === false && $is_guest === false && $is_restricted === false)) {
                ...
                if ($status) {
                    ...
                    $this->SOCOM_Git_Data_model->git_track_data(GitDataType::CANCEL_FILE, $file, $user_id);
        ...
        if ($data_check['result']) {
            ...
            if (in_array($tab_name, $tab_list) && ($is_pom_admin === false && $is_guest === false && $is_restricted === false)) {
                ...
                if ($status) {
                    ...
                    $this->SOCOM_Git_Data_model->git_track_data(GitDataType::DELETE_FILE, $file, $user_id);
