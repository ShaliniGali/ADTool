<?php
  ...
  $this->load->view('templates/essential_javascripts');
    ...
    // If dev bypass is enabled, show all tiles regardless of role
    if ($dev_bypass_enabled) {
        $show_import_upload = true;
        $show_coa_management = true;
        $show_cycle_management = true;
    } else {
        $show_import_upload = $is_restricted || $is_admin || $is_guest; // all types of users except restricted
        $show_coa_management = !$is_guest && !$is_restricted; // only for users and admins
        $show_cycle_management = $is_admin && ($is_cycle_admin_user || $is_weight_criteria_admin_user);
    }
