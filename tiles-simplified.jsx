...
// Simple Tab component replacement
const SimpleTab = ({ children, isActive, onClick }) => (
    <button 
        className={`btn ${isActive ? 'btn-primary' : 'btn-outline-primary'} me-2`}
        onClick={onClick}
    >
        {children}
    </button>
...
export const Tiles = () => {
    ...
    const onAppFilter = (tabIndex) => {
        setActiveTab(tabIndex)
        const tabKey = APP_FILTER_TABS[tabIndex]?.k || 'all'
        ...
        >
            ...
            >
                <div className="d-flex mb-3">
                    {APP_FILTER_TABS.map((t, index) => (
                        <SimpleTab 
                            key={t.k} 
                            isActive={activeTab === index}
                            onClick={() => onAppFilter(index)}
                        >
                            {t.label}
                        </SimpleTab>
                    ))}
